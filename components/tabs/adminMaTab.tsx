import { deleteMuridMa, getMuridMa, searchMuridMa, updateStatusMuridMa } from "@/app/api/muridMa";
...
export default function AdminMaTab(){
    const [muridMa, setMuridMa] = useState<any>([]);

    useEffect(() => {
        handleGetMuridMa()
    }, [])

    async function handleGetMuridMa(){
        const data = await getMuridMa()
        setMuridMa(data)
    }

    async function handleSearchMurid(search: string){
        const data = await searchMuridMa(search)
        setMuridMa(data)
    }

    async function handleDelete(id: string){
        const {success} = await deleteMuridMa(id)
        if(success){
    ...
    return(
        <>
            ...
            <div className="[&_.ant-table-cell]:text-xs overflow-y-auto h-[60vh]" data-aos="fade-left">
                <Table size="small" dataSource={muridMa} columns={columns} scroll={{ x: 'max-content' }} style={{fontSize: "8px"}} pagination={{
